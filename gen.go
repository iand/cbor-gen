...
func PrintHeaderAndUtilityMethods(w io.Writer, pkg string, typeInfos []*GenTypeInfo, scope string) error {
	var imports []Import
	...
	data := struct {
		...
		Scope         string
		MaxHeaderSize int
	}{pkg, imports, scope, maxHeaderSize}
...
var {{ .Scope }}_scratchPool = sync.Pool{
	New: func() interface{} {
...
var {{ .Scope }}_emptyScratch [{{ .MaxHeaderSize }}]byte
...
func emitCborMarshalStructMap(w io.Writer, gti *GenTypeInfo) error {
	...
	scratchPtr := cborgenmap_scratchPool.Get().(*[]byte)
	scratch := *scratchPtr
	defer func() {
		// Empty the buffer before returning. Not strictly necessary if we are
		// careful about what we read.
		copy(scratch, cborgenmap_emptyScratch[:])
		// Avoid scratch slice header from escaping to heap by reusing scratchPtr
		// which is already on the heap.
		*scratchPtr = scratch
		cborgenmap_scratchPool.Put(scratchPtr)
	}()
`)
	if err != nil {
		return err
